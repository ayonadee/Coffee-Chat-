...
@app.route('/deletenetwork/<int:user_id>')
def delete(user_id):
    network_to_delete = models.Network.query.filter_by(id=user_id).first()
...
def update(id):
    ...
    error=""
    form = NetworkForm()
    form.user_id.data = first_network.user_id
    ...
    if(request.method=='POST'):
        ...
        first_network.user_id = form.user_id.data
